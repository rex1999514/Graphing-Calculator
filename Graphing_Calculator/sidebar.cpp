...
void Sidebar::draw(sf::RenderWindow& window){
    ...
    for (vector<string>::iterator it = items.begin();
        ...
        equa_height.push_back(height + VERTICAL_LINE_SPACING);
...
string Sidebar::equation_pos(sf::Vector2f mouse)
{
    //find the equation position in the sidebar
    float high = SCREEN_HEIGHT / 10;
    float low = SCREEN_HEIGHT / 15;
    float block_height = (SCREEN_HEIGHT - high - low) / 25;
    if (mouse.x >= WORK_PANEL && mouse.x <= SCREEN_WIDTH)
    {
        //divide the equations into small blocks
        //use the height of each block and the y position of the equation to find its index
        if (mouse.x >= WORK_PANEL && mouse.x <= SCREEN_WIDTH)
        {
            if (mouse.y > high && mouse.y < SCREEN_HEIGHT - low)
            {
                int index = (mouse.y - high) / block_height + 3;
                return items[index];
            }
            else {
                return "nothing";
